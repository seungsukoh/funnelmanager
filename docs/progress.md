...
- 운영 UI 정리: 연결 설정을 별도 버튼 패널로 분리하고 메인 화면에 저장소/Google/테스트 발송/승인/다음 작업 상태 모니터링 추가
- 엑셀/CSV 명단 불러오기 추가: 브라우저에서 파일을 읽어 D1 명단에 저장하고 발송 승인 화면에서 선택 가능
- 엑셀/CSV 명단 UX 개선: 열 순서가 달라도 이메일/이름/메일 이름/퍼널 단계를 자동 판별하고 저장 전 미리보기에서 수정 가능
- 엑셀 읽기 오류 수정: `read-excel-file` 9.x의 여러 시트 반환값을 처리하고 실제 명단이 있는 시트를 자동 선택
- 메일 흐름 UX 개선: 메일 단계 추가/삭제, 후속 발송 없음/며칠 후/특정 날짜 예약, 다음 메일 선택을 화면에서 관리
- Gmail 결과 반영 자동 퍼널 진행 추가: `sent` 결과는 다음 메일 단계와 예약일로 이동하고 `failed` 결과는 재확인 대상으로 유지
- 명단 운영 기능 추가: 불러온 명단에서 체크한 사람만 발송 승인에 올리고, 선택 삭제/전체 삭제 시 관련 승인 목록과 Gmail 결과도 함께 정리

### 진행 중

- Outlook display 모드 사용자 테스트 준비
- 실제 Gmail Sheet 결과로 다음 발송일/다음 승인 목록 생성 검증 준비
- 실제 운영 명단으로 선택 발송/삭제 UX 검증 준비
