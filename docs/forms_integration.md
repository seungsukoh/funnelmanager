...
Cloudflare 화면의 `연결 설정 > 폼 자동 발송`을 켜면 새 폼 응답이 명단에 들어온 직후 첫 번째 메일
단계를 Gmail API로 발송한다. 이 옵션은 기본값이 꺼져 있으며, 켜기 전에 다음 항목을 확인해야 한다.

- `단계별 메일`의 첫 단계 제목/본문
- Google 연결 상태와 Gmail API 권한
- 하루 자동 발송 제한
- 테스트 폼 응답으로 실제 수신 확인

후속 메일 자동 발송은 같은 화면의 `예약일이 지난 후속 메일도 자동 발송`을 켠 뒤 예약 호출을 붙인다.
후속 발송 대상은 첫 메일 발송 후 고객 상태가 다음 단계로 이동했고, `next_send_at`이 오늘 이전인 고객만이다.
즉 `단계별 메일`에서 각 단계의 `며칠 후` 또는 `특정 날짜`와 `다음에 보낼 메일`을 선택해야 한다.

Google Apps Script 시간 트리거로 실행하려면 `integrations/google_forms_followup_scheduler.js` 내용을
같은 Apps Script 프로젝트에 붙여 넣고 다음 값을 수정한다.

```js
const FOLLOWUP_WEBHOOK_URL = "https://<cloudflare-pages-domain>/webhooks/send-due-followups";
const FOLLOWUP_WEBHOOK_TOKEN = "<FORM_WEBHOOK_TOKEN과 같은 값>";
```

그 다음 설치형 트리거에서 `runDueFollowups`를 시간 기반으로 실행한다. 예: 매일 오전 9시.
